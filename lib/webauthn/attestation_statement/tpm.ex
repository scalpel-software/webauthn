defmodule Webauthn.AttestationStatement.TPM do
  ...
  defp parse_pub_area(<<
    @tpm_alg_rsa::integer-size(16),
    name_alg::integer-size(16),
    object_attributes::integer-size(32),
    auth_policy_length::integer-size(16),
    auth_policy::binary-size(auth_policy_length),
    @tpm_alg_null::integer-size(16),
    scheme::integer-size(16),
    key_bits::integer-size(16),
    exponent::integer-size(32),
    unique_length::integer-size(16),
    unique::integer-unit(8)-size(unique_length)
  >>) do
    {:ok, %{
      type: :rsa,
      name_alg: name_alg,
      ...
      scheme: scheme,
      key_bits: key_bits,
      exponent: (if exponent == 0, do: 65537, else: exponent),
      unique: unique
    }}
  end
  ...
  defp parse_pub_area(<<
    ...
    @tpm_alg_null::integer-size(16),
    scheme::integer-size(16),
    curve_id::integer-size(16),
    @tpm_alg_null::integer-size(16),
    _unique_length::integer-size(16),
    x_length::integer-size(16),
    x::binary-size(x_length),
    y_length::integer-size(16),
    y::binary-size(y_length),
  >>) do
    {:ok, %{
      type: :ecc,
      name_alg: name_alg,
  ...
  defp create_attested_name(%CBOR.Tag{value: raw_pub_area}, pub_area) do
    ...
    |> (fn hash -> <<pub_area.name_alg::integer-size(16)>> <> hash end).()
  end
  ...
  defp valid_aik_manufacturer?({:rdnSequence, list}) do
    ...
    |> elem(2)
    |> String.slice(2..-1)
    |> (fn id -> MapSet.member?(@tcg_vendor_ids, id) end).()
  end
